...
def updategh():
    if not os.path.isfile(versionpath):
        print(Fore.RED + "  [-] Unable to check for updates: please re-clone the script to fix this problem" + Fore.RESET)
        sys.exit(1)

    print(" [*] Checking for updates...\n")
    conn = httplib.HTTPSConnection("raw.githubusercontent.com")
    conn.request("GET", "/MataGreek/greekhacking/core/version.txt")
    repoVersion = conn.getresponse().read().strip().decode()
    with open(versionpath) as vf:
            currentVersion = vf.read().strip()
    if repoVersion == currentVersion:print("  [*] The script is up to date!\n")
    else:
            print("  [+] An update has been found ::: Updating... ")
            conn.request("GET", "/MataGreek/greekhacking/greekhacking.py")
